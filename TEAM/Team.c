...
void operar_con_appeared_pokemon(t_appeared_pokemon * mensaje){
	...
	●Posición del Pokemon.*/
		...
		printf("Agregue el pokemon al mapa\n");
		planificar();
		free(pokemon);
...
t_list* obtener_pokemones(t_list* lista_global,t_list* lista, uint32_t posicion){
	...
	mostrar_pokemones_de_entrenador(lista_global, lista_pokemones_objetivos,posicion);

	return lista_pokemones_objetivos;

}

void mostrar_pokemones_de_entrenador(t_list* lista_global, t_list* lista_pokemones_objetivos, int posicion){
	t_objetivo_entrenador* objetivo = malloc(sizeof(t_entrenador*));
	for(uint32_t i = 0; i < list_size(lista_pokemones_objetivos); i++){
		objetivo = list_get(lista_pokemones_objetivos, i);
		if(objetivo == NULL){
			break;
		}
		if(lista_global == lista_global_pokemones){
			//log_info(team_logger, "Una especie de pokemon que tiene el entrenador %i es: %s y la cantidad es %i\n", posicion, objetivo->especie, objetivo->cantidad);
		} else {
			//log_info(team_logger, "Una especie de pokemon que quiere el entrenador %i es: %s y la cantidad es %i\n", posicion, objetivo->especie, objetivo->cantidad);
		}
		//free(objetivo);
...
Cuando todos los entrenadores dentro de un Team se encuentren en Exit, se considera que el proceso Team cumplió el objetivo global.
	...
	log_info(team_logger,"Proximo en ejecucion obtenido o esperando por otro entrenador para ejecutar");
